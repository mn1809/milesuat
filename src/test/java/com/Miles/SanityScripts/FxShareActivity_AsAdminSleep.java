...
public class FxShareActivity_AsAdminSleep extends MilesSettings {
...
// Read Values in Workout Details Page
		...
		String Duration = getWorkoutpageDuration();
...
// Compare the values 
		justSleepFor(6);
		Assert.assertEquals(Duration,getSleepDuration());
...
// Compare the values 
		justSleepFor(6);
		Assert.assertEquals(Integer.parseInt(sharelink.getBRValue()),BRValue);
		justSleepFor(6);
		Assert.assertEquals(HRValue,Integer.parseInt(sharelink.getHeartRateValue()));
	...
	protected String getWorkoutpageDuration()
	{
		 return driver.findElement(By.className("training-dur-dynamic")).getText();
	}
	...
	//This method will be used for to initateAdminPage.
		...
		{				//Prod
			 Info = "quality+sleepProd@fourthfrontier.com";
		}
		else
		{			//Stage
			 Info = "quality+sleep@fourthfrontier.com";
		}
	...
	protected void VerifySleepIcon()
	{		
	...
	{
		WebElement IconSleep = driver.findElement(By.className("icon-sleep"));
		Assert.assertTrue(IconSleep.isDisplayed());
	}
